...
**Status data is automatically generated.** The `/status` page reads from `public/data/status.json`, which is generated by the CryptoBot exporter.

To update status data:
1. Run the CryptoBot export script: `CryptoBot/tools/export_public_site_assets.py`
2. The script will update `public/data/status.json` and `public/data/status_history.jsonl`
3. Rebuild the site: `npm run build`
4. Commit and deploy

Do not manually edit `public/data/status.json` - your changes will be overwritten by the next export.
