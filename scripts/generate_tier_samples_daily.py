#!/usr/bin/env python3
"""
Generate Sample JSON/JSONL Files from Daily Tier Parquets

Pulls 5 random rows with all columns from each tier (all daily now) to create
human-readable sample files.

Usage:
    # Generate samples from R2 (default - uses latest date)
    python3 scripts/generate_tier_samples_daily.py

    # Specify output directory
    python3 scripts/generate_tier_samples_daily.py --output-dir output/samples

    # Use specific date for all tiers
    python3 scripts/generate_tier_samples_daily.py --date 2026-01-18

    # Use specific dates per tier
    python3 scripts/generate_tier_samples_daily.py --tier3-date 2026-01-18 --tier2-date 2026-01-17 --tier1-date 2026-01-17

Output:
    {output_dir}/tier1_sample_daily.jsonl  - 5 random rows from Tier 1 daily
    {output_dir}/tier2_sample_daily.jsonl  - 5 random rows from Tier 2 daily
    {output_dir}/tier3_sample_daily.jsonl  - 5 random rows from Tier 3 daily
"""
...
def get_tier_sample(s3_client, bucket: str, tier: int, date: Optional[str], n: int) -> pa.Table:
    """Get sample from any tier's daily parquet."""
    prefix = f"tier{tier}/daily/"
    
    if date is None:
        date = find_latest_date(s3_client, bucket, prefix)
        if date is None:
            raise RuntimeError(f"No Tier {tier} daily data found in R2")
    
    key = f"tier{tier}/daily/{date}/data.parquet"
    print(f"  Downloading {key}...")
    
    table = download_parquet(s3_client, bucket, key)
    print(f"  Source: {table.num_rows} rows, {table.num_columns} columns")
    
    return sample_table(table, n), date
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate sample JSONL files from daily tier parquets",
    ...
    parser.add_argument("--date", type=str, default=None,
                        help="Date for all tiers (YYYY-MM-DD), defaults to latest")
    parser.add_argument("--tier3-date", type=str, default=None,
                        help="Tier 3 date (YYYY-MM-DD), overrides --date")
    parser.add_argument("--tier2-date", type=str, default=None,
                        help="Tier 2 date (YYYY-MM-DD), overrides --date")
    parser.add_argument("--tier1-date", type=str, default=None,
                        help="Tier 1 date (YYYY-MM-DD), overrides --date")
    ...
    # Resolve dates
    tier3_date = args.tier3_date or args.date
    tier2_date = args.tier2_date or args.date
    tier1_date = args.tier1_date or args.date
    
    print("=" * 60)
    print("DAILY TIER SAMPLE GENERATOR (JSONL)")
    print("=" * 60)
    ...
    for tier_name in ['tier3', 'tier2', 'tier1']:
        ...
        if r:
            sample = r['sample']
            print(f"{tier_name.upper()}: {args.sample_size} rows, {sample.num_columns} columns (date: {r['date']})")
            print(f"        Columns: {sample.column_names}")
    
    print()
    print(f"Output files:")
    for f in sorted(output_dir.glob("tier*_sample_daily.jsonl")):
