...
def run_exporter(paths, scan_limit=None):
    ...
    if not paths["cryptobot_exporter"].exists():
        print(f"❌ ERROR: CryptoBot exporter not found at {paths['cryptobot_exporter']}")
        sys.exit(1)
    
    if not paths["cryptobot_archive"].exists():
        print(f"❌ ERROR: CryptoBot archive not found at {paths['cryptobot_archive']}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"❌ ERROR: Exporter failed with code {e.returncode}")
...
def verify_outputs(paths):
    ...
    if not status_json.exists():
        print(f"❌ ERROR: status.json not found at {status_json}")
        sys.exit(1)
    
    try:
        with open(status_json, 'r') as f:
            data = json.load(f)
        print(f"✓ Verified status.json ({len(json.dumps(data))} bytes)")
    except json.JSONDecodeError as e:
        print(f"❌ ERROR: status.json is not valid JSON: {e}")
        sys.exit(1)
    
    if not history_jsonl.exists():
        print(f"⚠️  Warning: status_history.jsonl not found (optional)")
    else:
        print(f"✓ Verified status_history.jsonl exists")
...
def generate_daily_update(paths, status_data, date_override=None, no_history=False):
    ...
    if update_file.exists():
        print(f"⚠️  Update file already exists: {update_file}")
...
"""
    ...
    print(f"✓ Generated daily update: {update_file}")
    return update_file
...
def main():
    ...
    print("🚀 Instrumetriq Site Publisher")
    print("=" * 60)
